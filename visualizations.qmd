...
```{python}
def SweepPlot(data, xName, yName, SweepColumnName, labelColumn, skipRows):
  plt.figure()
  n = data[SweepColumnName].max()
  n=n.astype(int)
  labels = dataset[labelColumn].dropna()
  ...
  prevIndex = 0
  for i in range(n):
    plt.plot(data[xName][skip+prevIndex:index[i]], data[yName][skip+prevIndex:index[i]])
    prevIndex = index[i]
...
dataset['Get Measurement Value'] = dataset['Get Measurement Value'] * 10**6
