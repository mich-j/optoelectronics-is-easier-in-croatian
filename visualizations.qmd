...
**suspended two plots because they had pretty much the same results as the next two ones**
...
We will fit an exponential function to our obtained results.
...
x=dataset['2 - Get Measurement Value'][3:44]
y = dataset['Get Measurement Value'][3:44]

popt, pcov = curve_fit(exp_func, x, y)

plt.plot(x, exp_func(x, *popt)*1000, label='fit')
...
```{python}
print(f'a = {popt[0]:.3E}, b = {popt[1]:.3f}')
```

```{python}
mu = q / (popt[1] * k * T)
print(f'\N{GREEK SMALL LETTER MU} = {mu:.3f}')
```
...
# Directory RED_LED_PD_mj/RED_LED_PD_mj/LED_RED_1mm_Ipd_UpdF_UpdR_02112022 -> the same as LED_PD_UpdF_UpdR_02112022

```{python}

dataset = pd.read_csv('datasets\RED_LED_PD_mj\RED_LED_PD_mj\LED_RED_1mm_Ipd_UpdF_UpdR_02112022\IpdF_UpdF 2022-12-02 16-24-39 0.csv', skiprows=[0,1,2,4,5,6,7], sep=',', usecols=['Time','Sweep CH2 Voltage Setting', 'Sweep CH1 Voltage Setting', 'Get Measurement Value', 'Get Measurement Value.1'])


pys.SweepPlot(dataset, 'Get Measurement Value.1', 'Get Measurement Value', 
  'Sweep CH2 Voltage Setting', 'Get Measurement Value', 2, yScale=10**6, labelScale=1000)
# plt.subplots_adjust()
plt.yscale('log')
plt.legend(title='$I_{LED} [mA]$', loc='best')
plt.suptitle('Photodiode forward voltage versus forward current')
...
dataset = pd.read_csv('datasets\RED_LED_PD_mj\RED_LED_PD_mj\LED_RED_1mm_Ipd_UpdF_UpdR_02112022\IpdR_UpdR 2022-12-02 15-57-44 0.csv', skiprows=[0,1,2,4,5,6,7], sep=',', usecols=['Time','Sweep CH2 Voltage Setting', 'Sweep CH1 Voltage Setting', 'Get Measurement Value', 'Get Measurement Value.1'])
...
## ILED_Green_Ipd_UpdUF_0_2V

Ipd_UpdUF_0_2V_G_Popt0_6V 2022-12-06 10-31-19 0.csv
```{python}
dataset = pd.read_csv('datasets\GREEN_LED_PD_mj\ILED_Green_Ipd_UpdUF_0_2V\Ipd_UpdUF_0_2V_G_Popt0_6V 2022-12-06 10-31-19 0.csv', skiprows=[0,1,2,3,4,5,7,8, 9], sep=',', usecols=['1 - Sweep CH2 Voltage Setting', '1 - Sweep CH1 Voltage Setting', '3 - Get Measurement Value', '2 - Get Measurement Value'])

# 'Sweep CH2 Voltage Setting', 'Sweep CH1 Voltage Setting', 'Get Measurement Value', 'Get Measurement Value.1'

pys.SweepPlot(dataset, '2 - Get Measurement Value', '3 - Get Measurement Value', 
  '1 - Sweep CH2 Voltage Setting', '2 - Get Measurement Value', 2, yScale=10**6, labelScale=1000)
# plt.subplots_adjust()
plt.yscale('log')
plt.legend(title='$I_{LED} [mA]$', loc='best')
plt.suptitle('Photodiode forward voltage versus forward current')
