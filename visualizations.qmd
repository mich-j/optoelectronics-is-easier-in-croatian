...
def SweepPlot(data:pd.DataFrame, xName: str, yName: str, SweepColumnName: str, 
  labelColumn: str, skipRows:int, xScale=1, yScale=1):

  n = data[SweepColumnName].max()
  n=n.astype(int)
  if(labelColumn!=None):
    labels = dataset[labelColumn].dropna().reset_index(drop=True)
...
dataset = pd.read_csv('datasets/LED_PD_UpdF_UpdR_02112022/IpdF_UpdF_2022-12-02_16-24-39_0.csv',
  sep=',', skiprows=3)
dataset['Get Measurement Value'] = dataset['Get Measurement Value'] * 10**6 #to nA
SweepPlot(dataset, 'Get Measurement Value.1', 'Get Measurement Value', 
  'Sweep CH2 Voltage Setting', 'Get CH2 Current Measurement', 2)
plt.yscale('log')
plt.legend(title='I', loc='best')
plt.suptitle('Photodiode forward voltage versus for ward current')
plt.title('No illumination')
...
SweepPlot(dataset, 'Get Measurement Value.1', 'Get Measurement Value', 'Sweep CH2 Voltage Setting', 'Get CH2 Current Measurement', 2)
plt.legend(title='I', loc='best')
...
SweepPlot(dataset, '2 - Get Measurement Value', '1 - Get Measurement Value', '3 - Sweep CH2 Voltage Setting', '3 - Get CH2 Current Measurement', 2, yScale=10**6)
plt.legend(title='$I_{LED}$', loc='best')
plt.yscale('log')
plt.suptitle('Photodiode voltage versus photodiode current')
plt.title('Variable LED illumination')
plt.xlabel('$U_{pd} [V]$')
plt.ylabel('$I_{pd} [nA]$')
...
plt.title('Photodiode voltage versus forward current')
